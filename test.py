...
class GranMSimplexFractions:
    ...
    def mixed_value_to_html(self, value):
        ...
        if value.coefficient == 0 and value.M_coefficient == 0:
            ...
            if value.M_coefficient == 1:
                ...
                return f"{self.fraction_to_html(value.M_coefficient)}M"
        else:
            coef_str = self.fraction_to_html(value.coefficient)
            if value.M_coefficient == 1:
                m_str = "M"
            elif value.M_coefficient == -1:
                m_str = "-M"
            ...
            if i != pivot_row:
                ...
                if factor.coefficient != 0 or factor.M_coefficient != 0:
                    ...
                    for j in range(len(matrix[0])):
                        ...
                        var_name = all_var_names[j] if j < len(all_var_names) else 'RHS'
                        self.html_output += f"<li>{var_name}: {self.mixed_value_to_html(old_val)} - ({self.mixed_value_to_html(factor)} × {self.mixed_value_to_html(pivot_val)}) = {self.mixed_value_to_html(matrix[i][j])}</li>"
                    self.html_output += "</ul>"
    ...
    def create_table_html(self, matrix: List[List], all_var_names: List[str], 
        ...
        for i in range(len(matrix)):
            ...
            for j in range(len(matrix[0])):
                ...
                html += f'<td{css_class}>{self.mixed_value_to_html(matrix[i][j])}</td>'
            html += '</tr>'
    ...
    def solve(self, objective: List[float], constraints: List[List[float]], 
        ...
        <body>
            <h1>Método de la Gran M - Cálculos Exactos con Fracciones</h1>
        """
        ...
        if artificial_in_basis:
            ...
            z_value = matrix[-1][-1]
            if not minimize:
                z_value = MixedValue(-z_value.coefficient, -z_value.M_coefficient)
            
            self.html_output += f'<p><strong>Valor {"mínimo" if minimize else "máximo"} de la función objetivo: {self.mixed_value_to_html(z_value)}</strong></p>'
...
def test_fractions():
    ...
    print("=== MÉTODO DE LA GRAN M CON FRACCIONES EXACTAS ===")
    
    # Ejemplo con fracciones
    print("\nEjemplo con fracciones:")
    print("Minimizar Z = (1/2)x1 + (3/4)x2")
    print("Sujeto a:")
    print("  (2/3)x1 + x2 >= 5/2")
    print("  x1 + (1/3)x2 >= 4/3")
    print("  x1, x2 >= 0")
    ...
    with open("gran_m_fractions.html", "w", encoding="utf-8") as f:
        f.write(html_report)
    
    print("✅ Reporte con fracciones generado: gran_m_fractions.html")
